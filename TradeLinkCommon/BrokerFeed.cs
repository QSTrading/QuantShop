...
namespace TradeLink.Common
{
    ...
    public class BrokerFeed : TLClient
    {
        ...
        public void Reset()
        {
            ...
            /*
             * //用事后点击事件调用initfeeds与后台直接加载initfeeds会出现时间不同程度的延迟,造成函数 如果放入线程中运行就会正常
            v("TLClientName:" + tl.Name);
            v("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
            //tl.Mode(0, false);
            v("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
            //检查TLClient所连接 对应的TLServer的Feature是否支持交易或者数据 //用事后点击事件调用initfeeds与后台直接加载initfeeds会出现时间不同程度的延迟,造成函数
            v(hasminexec(tl, 0).ToString());
            v("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
            //Thread.Sleep(2000);
            v("brokername:"+tl.BrokerName.ToString());
            v("tlcient future list count:" + tl.RequestFeatureList.Count.ToString());
            v("tl providerselected:" + tl.ProviderSelected.ToString());
            */
            ...
            for (int i = 0; i < ProvidersAvailable.Length; i++)
            {
                if ((qi != -1) && (xi != -1)) break;
                // switch to provider
                //v("quote function:" + hasminquote(tl, i).ToString());
                //v("execution function:" + hasminexec(tl, i));
            ...
            // not allowed
            v("requirepreferred:" + RequirePreferred.ToString());
            v("q x:" + qi.ToString() + " " + xi.ToString());
            //tl searchclient使用完毕后 正常退出connection
            //tl.Disconnect();
            //return;
            if (RequirePreferred)
            {
        ...
        {
            v("getsearchclient 返回用于搜索服务端的客户端");
            ...
            {
                v("实例化通讯TLClient 客户端");
                TLClient_IP tmp = new TLClient_IP(_servers, _port,SendDebugEvent);
                //tmp.VerboseDebugging = VerboseDebugging;
                //tmp.SendDebugEvent+=new DebugDelegate(debug);
                return tmp;
            }
        ...
        TLClient getrealclient(int pidx, string name)
        {
            ...
            v("getrealclient 返回realclient");
            if (!IPUtil.hasValidAddress(_servers))
            {
                TLClient_WM tmp = new TLClient_WM(pidx, name, false);
                //tmp.SendDebugEvent +=new DebugDelegate(v);
                return tmp;
            }
            else
            {
