           ...
           //_started=false
            try
            {
                ...
                if (_bw2.IsBusy)
                    _bw2.CancelAsync();
                //return;
                if (server.Connected && _started) //如果实现已经stop了brokerfeed 会造成服务器循环相应。应该将_stated放在这里进行相应
                {
                    _started = false;
                    try
                    {
                        ...
                        server.Close();
                        //server = null;
                        //debug("disconnect connected:"+server.Connected.ToString());
                    }
                    catch { }
                }
        ...
        */
        ...
        bool connect(int providerindex, bool showwarn)
        {
            ...
            try
            {
                ...
                //debug("servers shutdown" + ((server != null) && (server.Connected)).ToString());
            ...
            {
                v("exception creating connection to: " + serverip[providerindex].ToString());
                v(ex.Message + ex.StackTrace);
                _connect = false;
            }
            //这里的连接并没有完全执行完毕,mod里还有启动线程等其他工作
            /*
            if (_connect && _b.Count > 0)
            {
                //v("resubscribing basket: " + _b.ToString());
                //Register();
                //Subscribe(_b);
                //updateheartbeat();
            }*/
        ...
        /// <summary>
        /// 关于socket的连接
        /// Server - Client 当Client发起一个连接 server做出相应后 如果双方都没有close socket则连接会一致存在
        /// Client socket.close()在收到数据后主动关闭一个连接 连接会进入Close_Wait状态 然后服务端的ReadData会一致循环csi.sock.BeginReceive 准备进入读取更多的数据
        /// Client 方式2关闭 程序进入假死状态 ，服务端任然循环 方式错误
        /// Server 方式2关闭 进入Close_wait Fin_wait_2
        /// 双向关闭 服务端方式1 客户端方式2/1 则会进入time_wait状态 表明正常关闭
        /// </summary>
        /// <returns></returns>
        public Providers [] TLFound()
        {
            ...
            foreach (IPEndPoint ep in serverip)
            {
                try
                {
                    ...
                    //TLFund打开了一个socket用于获取TLServer端的信息,使用完毕后需要安全关闭
                    //1.直接关闭连接
                    s.Close();
                    //2.干净关闭连接
                    ...
                    try
                    {
                        ...
                        {
                            debug("Found provider: " + p.ToString() + " at: " + ep.ToString());
