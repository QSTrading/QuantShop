...
namespace TradeLink.Common
{
    ...
    public class TLServer_WM : Form, TLServer
    {
        ...
        public void newTick(Tick tick)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < client.Count; i++) // send tick to each client that has subscribed to tick's stock
                    ...
                    {
                        debug("Notifies subscribed clients of a new tick."+"client:"+hims[i].ToString());
        ...
        {
            //每个client维护者client,heart,stocks,index,hims,列表的信息 每个cleint都一个对应的数据项 共同
            //组成一个client的数据集合
            ...
            debug(WMUtil.HisHandle(cname).ToString());
        }
        ...
        void SrvRegStocks(string cname, string stklist)
        {
            int cid = client.IndexOf(cname);
            if (cid == -1) return;
            stocks[cid] = stklist;//某个client所注册的symbol列表 sym1,sym2,sym3...
            SrvBeatHeart(cname);//发送心跳信息
            if (newRegisterSymbols != null) newRegisterSymbols(cname, stklist);//通过回调请求Symbol 数据
        ...
        protected override void WndProc(ref System.Windows.Forms.Message m)
        {
            ...
            switch (tlm.type)
            {
                case MessageTypes.ACCOUNTREQUEST:
                    ...
                    string a="demo";
                    int p=123;
                    string accts = newAcctRequest(a,p);
