...
namespace TradeLink.Common
{
    ...
    public class TickArchiver
    {
        ...
        public bool newTick(Tick t)
        {
            ...
            int lastdate = 0;
            // get last date 每个Symbol有个最新的时间存放在datedict
            bool havedate = datedict.TryGetValue(t.symbol, out lastdate);
            // if we don't have date, use present date如果该symbol没有日期则我们新建一个日期
            if (!havedate)
            {
                lastdate = t.date;
            ...
            // see if we have stream already 通过symbol得到对应的tickwriter(写入tick数据)
