...
namespace TradingLib.Core
{
    ...
    /// 虚拟broker用于将单个账户虚拟成多个账户,多个虚拟账户有自己独立的结算与交易.该broker将所有的order通过一个broker账户结算与交易
    /// </summary>
    public class MiddleBroker
    {
        ...
        /// <summary>
        /// Occurs when [got order]. 当Broker得到Order时,我们出发该事件
        /// </summary>
        ...
        private IBroker _remoteBroker = null;

        public MiddleBroker(IBroker r)
        {
            _remoteBroker = r;
            Reset();
        ...
        //Broker记录一个Order,并且需要向外部真实Broker发送该Order
        protected void AddOrder(Order o, Account a)
        {
            debug("MBroker AddOrder Into OrderBook");
            ...
            //通过IBroker将本地Order发送出去
            
            //
            debug("Route Order To Remote Broker");
            //_remoteBroker.SendOrder(o);
        ...
        /// 
        /// client将Order送到broker,Broker需要记录下该Order
        /// </summary>
        /// <param name="o"></param>
